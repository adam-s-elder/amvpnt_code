...
run_test <- function(vaccinees, num_mc_pval = 100, num_nrm_dst = 100){
  ...
  lp_control <- amp::test.control(
    big_train = TRUE, f_cv_summary = mean, more_info = TRUE,
    n_bs_smp = num_nrm_dst, nrm_type = "lp", nrmlize = FALSE,
    num_folds = 1, perf_meas = "mag",
    pos_lp_norms = c(1, 2, 4, 6, 8, 10, "max"), show_hist = FALSE,
    test_stat_func = "mag", ld_est_meth = "par_boot",
    ts_ld_bs_samp = num_mc_pval
  )
  tst_res_lp <- cv_test(obs_data = vaccinees,
  ...
  lp_control$pos_lp_norms <- 2
  tst_res_l2 <- cv_test(obs_data = vaccinees,
                        param_est = amp::ic.data.examp,
                        control = l2_control)
  ...
  tst_res_max <- cv_test(obs_data = vaccinees,
                        param_est = amp::ic.data.examp,
  ...
  ssq_control$nrm_type <- "ordl2"
  tst_res_ssq <- cv_test(obs_data = vaccinees,
                         param_est = amp::ic.data.examp,
                         control = ssq_control)
  ...
  ## Running a Bonferroni correction based test
  start_time <- Sys.time()
  tst_res_bonf <- "NOTHING"
  cat("time for bonf test: \n")
  bonf_time <- Sys.time() - start_time
  print(bonf_time)
